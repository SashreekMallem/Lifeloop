...
/**
 * @fileOverview Defines Zod schemas and Genkit tools for interacting with Google Calendar.
 * This file does NOT use 'use server' and can export objects like Zod schemas and tools.
 */
...
export const BaseEventInputSchema = z.object({
...
export const CalendarActionStatusSchema = z.discriminatedUnion("status", [
  z.object({
...
// 1. Get Calendar Events Schemas
export const GetCalendarEventsInputSchema = BaseEventInputSchema.extend({
...
export const GetCalendarEventsOutputSchema = z.discriminatedUnion("status", [
  z.object({
    status: z.literal("success"),
...
// 2. Add Calendar Event Schema
export const AddCalendarEventInputSchema = BaseEventInputSchema.extend({
...
// 3. Edit Calendar Event Schema
export const EditCalendarEventInputSchema = BaseEventInputSchema.extend({
...
// 4. Delete Calendar Event Schema
export const DeleteCalendarEventInputSchema = BaseEventInputSchema.extend({
...
export const getActualCalendarEventsTool = ai.defineTool(
  ...
  async (input) => {
    ...
    try {
      ...
      const events = mappedEvents.filter((event): event is z.infer<typeof EventSchema> => event !== null);
...
export const addCalendarEventTool = ai.defineTool(
  ...
  async (input) => {
    ...
    try {
      ...
      const createdEvent: z.infer<typeof EventSchema> = {
        id: responseData.id,
...
export const editCalendarEventTool = ai.defineTool(
  ...
  async (input) => {
    ...
    try {
      ...
      const updatedEvent: z.infer<typeof EventSchema> = {
        id: responseData.id,
        summary: responseData.summary || "No Title",
        description: responseData.description,
        start: {dateTime: responseData.start?.dateTime || responseData.start?.date, timeZone: responseData.start?.timeZone},
