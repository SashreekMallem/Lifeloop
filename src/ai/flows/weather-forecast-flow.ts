...
// Input and Output Schemas (remain the same for the widget)
...
const WeatherApiToolOutputSchema = z.object({
  actualLocationName: z.string().describe("The actual name of the location found by the geocoding service."),
  latitude: z.number(),
  longitude: z.number(),
...
const getWeatherFromApiTool = ai.defineTool(
  {
    ...
    description: 'Fetches real-time weather data for a given location using Open-Meteo API.',
    inputSchema: z.object({
      location: z.string().describe('The city and optionally state/country to get weather for (e.g., "Berlin, Germany", "San Francisco").'),
    }),
  ...
  async (input) => {
    try {
      ...
      const { latitude, longitude, name: actualLocationName, admin1, country } = geoData.results[0];
      const displayLocationName = `${actualLocationName}${admin1 ? ', ' + admin1 : ''}${country ? ', ' + country : ''}`;
      ...
      const weatherResponse = await fetch(forecastUrl);
       if (!weatherResponse.ok) {
        console.error(`Weather API error for ${displayLocationName}: ${weatherResponse.status}`);
      ...
      // It's often better to let the LLM know the tool failed rather than trying to mask it completely,
      // or to provide a very specific error structure if the prompt is designed to handle it.
      // For now, we re-throw, and the flow can decide on a general fallback.
      // Alternatively, you could return a specific error object if the outputSchema allows for it.
...
User wants weather for: {{{location}}}

Instructions:
1. ALWAYS use the 'getWeatherFromApiTool' with the provided 'location' to fetch current weather data.
2. From the tool's output (which includes actualLocationName, temperature, weatherCode, humidity, windSpeed):
    a. Set 'locationName' to the 'actualLocationName' returned by the tool.
    ...
    d. Based on the 'weatherCode' and derived 'condition', select the MOST appropriate 'icon' string from this EXACT list: "Sun", "CloudSun", "Cloud", "CloudRain", "CloudSnow", "CloudLightning", "Wind", "Thermometer".
...
If the tool provides a generic location because the original was not found, use that generic location's data.
`,
});
...
const weatherForecastFlow = ai.defineFlow(
  ...
  async (input) => {
    try {
      ...
      // Validate the output, especially the icon
      ...
      if (!output.icon || !validIcons.includes(output.icon as any)) {
        console.warn(`LLM returned invalid icon: ${output.icon}. Defaulting to Thermometer.`);
        // Attempt to return the rest of the output if it's mostly valid
        return {
          ...
          icon: "Thermometer", // Force valid icon
          ...
          ...output, // Spread valid parts, then override icon again
          icon: "Thermometer"
        };
      }
      return output;
      ...
      // Generic fallback if the flow or tool throws an unhandled error
      return {
        locationName: input.location,
        temperature: "N/A",
        condition: "Forecast unavailable due to system error.",
        icon: "AlertTriangle" in iconComponents ? "AlertTriangle" : "Thermometer", // Use AlertTriangle if available, else Thermometer
        humidity: "N/A",
        wind: "N/A",
...
// Helper to map Lucide icon names to components (used by WeatherWidget, not directly in flow)
// This is illustrative; the actual mapping happens in the widget.
const iconComponents: { [key: string]: string } = {
  ...
  Wind: "Wind", // Assuming Wind is a Lucide icon name
